...
router.post('/tests', wrap(async (req, res) => {
  ...
  const id = uuidv4();
  await db.run(
    'INSERT INTO tests (id, title, description, duration, total_marks, instructions, created_by) VALUES (?, ?, ?, ?, ?, ?, ?)',
...
// Get all tests
router.get('/tests', wrap(async (req, res) => {
...
router.get('/tests/:id', wrap(async (req, res) => {
  ...
  for (const q of questions) {
    q.sample_cases = await db.all(
      'SELECT * FROM sample_test_cases WHERE question_id = ?',
      [q.id]
    );
    q.hidden_cases = await db.all(
      'SELECT * FROM hidden_test_cases WHERE question_id = ?',
      [q.id]
    );
    q.boilerplate = await db.all(
      'SELECT * FROM boilerplate_code WHERE question_id = ?',
      [q.id]
    );
  }
...
router.put('/tests/:id', wrap(async (req, res) => {
  ...
  await db.run(
    'UPDATE tests SET title=?, description=?, duration=?, total_marks=?, instructions=?, is_active=? WHERE id=?',
...
router.delete('/tests/:id', wrap(async (req, res) => {
  ...
  const questions = await db.all('SELECT id FROM questions WHERE test_id = ?', [testId]);

  for (const q of questions) {
    await db.run('DELETE FROM sample_test_cases WHERE question_id = ?', [q.id]);
    await db.run('DELETE FROM hidden_test_cases WHERE question_id = ?', [q.id]);
    await db.run('DELETE FROM boilerplate_code WHERE question_id = ?', [q.id]);
  }

  await db.run('DELETE FROM questions WHERE test_id = ?', [testId]);

  const attempts = await db.all('SELECT id FROM test_attempts WHERE test_id = ?', [testId]);
  for (const a of attempts) {
    await db.run('DELETE FROM question_submissions WHERE attempt_id = ?', [a.id]);
  }
  await db.run('DELETE FROM test_attempts WHERE test_id = ?', [testId]);
...
// Add question
router.post('/tests/:testId/questions', wrap(async (req, res) => {
  const {
    title,
    problem_statement,
    input_format,
  ...
  if (sample_cases) {
    for (const tc of sample_cases) {
      await db.run(
        'INSERT INTO sample_test_cases (id, question_id, input, expected_output, explanation) VALUES (?, ?, ?, ?, ?)',
  ...
  if (hidden_cases) {
    for (const tc of hidden_cases) {
      await db.run(
        'INSERT INTO hidden_test_cases (id, question_id, input, expected_output) VALUES (?, ?, ?, ?)',
  ...
  if (boilerplate) {
    for (const bp of boilerplate) {
      await db.run(
        'INSERT INTO boilerplate_code (id, question_id, language, code) VALUES (?, ?, ?, ?)',
  ...
  const totalRow = await db.get(
    'SELECT COALESCE(SUM(marks),0) as t FROM questions WHERE test_id = ?',
    [req.params.testId]
  );
  await db.run('UPDATE tests SET total_marks = ? WHERE id = ?', [totalRow.t, req.params.testId]);
...
router.put('/questions/:id', wrap(async (req, res) => {
  ...
  const existingQuestion = await db.get('SELECT test_id, order_index FROM questions WHERE id = ?', [req.params.id]);
  const nextOrderIndex = order_index ?? existingQuestion?.order_index ?? 0;

  await db.run(
    'UPDATE questions SET title=?, problem_statement=?, input_format=?, output_format=?, constraints=?, marks=?, order_index=?, image_url=? WHERE id=?',
    [title, problem_statement, input_format || '', output_format || '', constraints || '', marks, nextOrderIndex, image_url || null, req.params.id]
  );

  await db.run('DELETE FROM sample_test_cases WHERE question_id = ?', [req.params.id]);
  if (sample_cases) {
    for (const tc of sample_cases) {
      await db.run(
        'INSERT INTO sample_test_cases (id, question_id, input, expected_output, explanation) VALUES (?, ?, ?, ?, ?)',
  ...
  await db.run('DELETE FROM hidden_test_cases WHERE question_id = ?', [req.params.id]);
  if (hidden_cases) {
    for (const tc of hidden_cases) {
      await db.run(
        'INSERT INTO hidden_test_cases (id, question_id, input, expected_output) VALUES (?, ?, ?, ?)',
  ...
  await db.run('DELETE FROM boilerplate_code WHERE question_id = ?', [req.params.id]);
  if (boilerplate) {
    for (const bp of boilerplate) {
      await db.run(
        'INSERT INTO boilerplate_code (id, question_id, language, code) VALUES (?, ?, ?, ?)',
  ...
  if (q) {
    ...
    await db.run('UPDATE tests SET total_marks = ? WHERE id = ?', [totalRow.t, q.test_id]);
  }
...
router.delete('/questions/:id', wrap(async (req, res) => {
  const q = await db.get('SELECT test_id FROM questions WHERE id = ?', [req.params.id]);
...
// Get test results
router.get('/tests/:id/results', wrap(async (req, res) => {
...
router.get('/attempts/:attemptId', wrap(async (req, res) => {
...
router.get('/dashboard', wrap(async (req, res) => {
  const totalStudents = (await db.get("SELECT COUNT(*) as c FROM users WHERE role='student'"))?.c || 0;
  const totalTests = (await db.get('SELECT COUNT(*) as c FROM tests'))?.c || 0;
  const totalAttempts = (await db.get('SELECT COUNT(*) as c FROM test_attempts'))?.c || 0;
...
router.delete('/reset/all', wrap(async (req, res) => {
  await db.run('DELETE FROM question_submissions');
  await db.run('DELETE FROM test_attempts');
  await db.run("DELETE FROM users WHERE role = 'student'");
  res.json({ message: 'All student data reset' });
}));

// Reset only attempts
router.delete('/reset/attempts', wrap(async (req, res) => {
  await db.run('DELETE FROM question_submissions');
  await db.run('DELETE FROM test_attempts');
...
router.delete('/students/:id', wrap(async (req, res) => {
  ...
  await db.run('DELETE FROM test_attempts WHERE student_id = ?', [req.params.id]);
