...
// Start / resume test
router.post('/tests/:id/start', wrap(async (req, res) => {
  const test = await db.get('SELECT * FROM tests WHERE id = ? AND is_active = 1', [req.params.id]);
  ...
  await db.run(
    'INSERT INTO test_attempts (id, test_id, student_id, started_at) VALUES (?, ?, ?, ?)',
...
router.get('/attempts/:attemptId/questions', wrap(async (req, res) => {
  ...
  for (const q of questions) {
    ...
    q.saved = await db.get(
      'SELECT language, code FROM question_submissions WHERE attempt_id = ? AND question_id = ?',
      [req.params.attemptId, q.id]
    ) || null;

    const hiddenCount = (await db.get(
      'SELECT COUNT(*) as c FROM hidden_test_cases WHERE question_id = ?',
      [q.id]
    ))?.c || 0;
...
router.post('/run', wrap(async (req, res) => {
  ...
  const question = await db.get('SELECT id FROM questions WHERE id = ?', [question_id]);
  if (!question) return res.status(404).json({ error: 'Question not found' });

  const sampleCases = await db.all('SELECT * FROM sample_test_cases WHERE question_id = ?', [question_id]);
...
router.post('/attempts/:attemptId/submit-question', wrap(async (req, res) => {
  ...
  const question = await db.get(
    'SELECT marks FROM questions WHERE id = ? AND test_id = ?',
    [question_id, attempt.test_id]
  );
  if (!question) return res.status(404).json({ error: 'Question not found for this attempt' });

  const maxMarks = question?.marks || 10;

  let evalCases = await db.all(
    'SELECT * FROM hidden_test_cases WHERE question_id = ?',
    [question_id]
  );
  if (!evalCases.length) {
    evalCases = await db.all('SELECT * FROM sample_test_cases WHERE question_id = ?', [question_id]);
  }
...
// Final submit
router.post('/attempts/:attemptId/submit', wrap(async (req, res) => {
...
// Auto-save
router.post('/attempts/:attemptId/autosave', wrap(async (req, res) => {
  const { question_id, language, code } = req.body;
  const attempt = await db.get(
    'SELECT id, test_id FROM test_attempts WHERE id = ? AND student_id = ? AND status != ?',
...
// Tab switch
router.patch('/attempts/:attemptId/tab-switch', wrap(async (req, res) => {
  await db.run(
    'UPDATE test_attempts SET tab_switches = tab_switches + 1 WHERE id = ? AND student_id = ?',
    [req.params.attemptId, req.user.id]
  );
  const attempt = await db.get('SELECT tab_switches FROM test_attempts WHERE id = ?', [req.params.attemptId]);
