...
async function seed() {
  ...
  const existing = await get("SELECT id FROM tests WHERE title = 'Campus Placement - Round 1'");
  if (existing) {
  ...
  const testId = uuidv4();
  // Q1: 2 hidden cases x 5 marks = 10, Q2: 2 hidden x 5 = 10, Q3: 2 hidden x 5 = 10 -> total 30
  await run(
    `INSERT INTO tests (id, title, description, duration, total_marks, instructions, is_active)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [
      testId,
  ...
  const q1 = uuidv4();
  await run(
    `INSERT INTO questions (id, test_id, title, problem_statement, input_format, output_format, constraints, marks, order_index)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      q1, testId,
      'Sum of Two Numbers',
  ...
  await run(
    'INSERT INTO sample_test_cases (id, question_id, input, expected_output, explanation) VALUES (?, ?, ?, ?, ?)',
    [uuidv4(), q3, '10', 'NO', '10 = 2 x 5, not prime']
  );
  await run(
    'INSERT INTO hidden_test_cases (id, question_id, input, expected_output) VALUES (?, ?, ?, ?)',
    [uuidv4(), q3, '1', 'NO']
